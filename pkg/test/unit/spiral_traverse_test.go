...
func TestSpiralTraversr(t *testing.T) {
	Convey("Test SpiralTraversr", t, func(c C) {
		absPath, err := filepath.Abs("../test_data/spiral_traverse.json")
		...
		for i, testCase := range testCases {
			Convey(fmt.Sprintf("Test %v:", i+1), func(c C) {
				output := arrays.SpiralTraverse(testCase.NestedArrayInt)
